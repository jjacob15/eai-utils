...
var RadarChart = {
    draw: function (el, d, options) {
        ...
        //increase the max value by 1% to make it look cleaner
        cfg.maxValue = cfg.maxValue * 1.1;

        var allAxis = (d[0].map(function (i) { return i.axis }));
        var total = allAxis.length;
        var pointRadius = cfg.factor * Math.min(cfg.w / 2, cfg.h / 2);
        // var Format = d3.format(',');
        var Format = function (val) {
            var num = +d3.format(',')(val);
        ...
        for (var j = 0; j < cfg.levels ; j++) {
            var levelFactor = cfg.factor * pointRadius * ((j + 1) / cfg.levels);
            g.selectAll(".levels")
                .data(allAxis)
                .enter()
                .append("svg:line")
