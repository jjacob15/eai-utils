    ...
    return (
      <React.Fragment>
        <a className={`dropdown-item list-item ${cursor === i ? 'active' : ''}`} onClick={this.handleClick}>
          <div className={`checkbox-fade fade-in-${cursor === i ? 'primary' : 'default'}`} style={{ marginRight: '5px' }}>
  ...
  render() {
    ...
    return (
      <div className="dropdown input-group-btn">
        ...
        <button className="btn btn-primary btn-sm-icon dropdown-toggle" onKeyDown={this.handleKeyDown}
          
          onClick={this.showDropDown}>
          <i className={icon ? icon : ''}></i>
          </button>
        <div className="dropdown-menu dropdown-menu-right" style={{ display: show ? 'block' : 'none' }}>
          {localData &&
